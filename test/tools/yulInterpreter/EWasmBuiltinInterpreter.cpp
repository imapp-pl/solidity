...
u256 EWasmBuiltinInterpreter::evalBuiltin(YulString _fun, vector<u256> const& _arguments)
{
	...
	{
		accessMemory(arg[0], 8);
		readMemoryWord(arg[0]);
	}
	else if (_fun == "drop"_yulstring)
		return {};
	else if (_fun == "invalid"_yulstring)
		throw ExplicitlyTerminated();
	else if (_fun == "eth.getAddress"_yulstring)
		return writeU256(arg[0], m_state.address);
		...
		return writeU256(arg[0], m_state.balance);
	...
	{ /* TODO */ return {}; }
		...
		yulAssert(false, "Unknown builtin: " + _fun.str());
...
{
	for (size_t i = 0; i < 8; i++)
		m_state.memory[_offset + i] = uint8_t((_value >> i) & 0xff);
}

u256 EWasmBuiltinInterpreter::writeU256(uint64_t _offset, u256 const& _value)
{
